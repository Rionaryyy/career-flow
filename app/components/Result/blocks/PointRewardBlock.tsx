...
export default function PointRewardBlock({ plan, answers }: Props) {
  ...
  if (
    (b.paymentReward ?? 0) === 0 &&
    (b.carrierBarcodeReward ?? 0) === 0 &&
    (b.carrierShoppingReward ?? 0) === 0 &&
    (b.pointReward ?? 0) === 0 &&
  ...
  return (
    <div className="mt-3 border-t border-dashed border-gray-300 pt-2">
        ...
        answers.phase2.shoppingEcosystem.length > 0 && (
          <p className="ml-2 text-gray-700">
            ...
            {answers.phase2.shoppingEcosystem
              .map((eco: string) => eco.toUpperCase())
              .join(" / ")}
          </p>
        )}
      ...
      <div className="ml-2 space-y-1 text-gray-700 text-sm mt-1">
        ...
        {(b.carrierBarcodeReward ?? 0) > 0 && (
          <p>📱 バーコード決済還元: -¥{b.carrierBarcodeReward.toLocaleString()}</p>
        )}
        {(b.carrierShoppingReward ?? 0) > 0 && (
          <p>🛍 ショッピング利用還元: -¥{b.carrierShoppingReward.toLocaleString()}</p>
        )}
        {(b.pointReward ?? 0) > 0 && (
      ...
      {/* 🎁 合計 */}
      {(b.effectiveReward ?? 0) > 0 && (
        <p className="ml-2 mt-2 font-semibold text-green-700">
