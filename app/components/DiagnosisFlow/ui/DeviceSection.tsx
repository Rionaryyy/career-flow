  ...
  return (
    <div className="w-full space-y-6 py-6">
      {deviceQuestions.map((q) => {
        ...
        // ✅ deviceStorage は最後で個別描画するためスキップ
                    ...
                    >
                      ...
                      {isOpen && (
                        <div className="p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                          {series.models.map(
                            (model: { label: string; storage: string[] }) => (
                              <div
                                key={model.label}
      ...
      {(() => {
        ...
        const selectedModel = deviceList
          .flatMap(
            (s: { models: { label: string; storage: string[] }[] }) => s.models
          )
          .find((m: { label: string }) => m.label === answers.deviceModel);
