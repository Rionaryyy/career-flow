...
// ===================================================
// 📱 端末フィルター（allPlansのdeviceModel使用版 / 型修正版）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  // === 2️⃣ キャリア購入 or 返却プログラム ===
    ...
    if (selectedModel) {
      ...
      filtered = filtered.filter((plan) => {
        const modelStr = plan.deviceModel ?? ""; // 🔸 Plan型に追加したdeviceModelを参照
        const models = modelStr
          .split(",")
          .map((m: string) => m.toLowerCase().trim().replace(/\s+/g, "")); // ✅ 型付け済み

        return (
          models.some((m: string) => m.includes(normalizedModel)) ||
          models.some((m: string) => normalizedModel.includes(m))
        );
      });
    }
  }
