...
// ===================================================
// 📱 端末フィルター（iPhone / Pixel / Galaxy / Xperia対応・Google補完）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      // ✅ ベースシリーズ抽出
      const baseSeries =
        normalizedModel.match(/(iphone|galaxy|(google)?pixel|xperia)\d+/)?.[0] ??
        normalizedModel;

      // ✅ "Google"付きPixelやFE/Fold等の亜種対応
      const seriesRegex = new RegExp(
        `^(google)?${baseSeries}(pro|plus|ultra|fold|a|fe|mini)?$`
      );
