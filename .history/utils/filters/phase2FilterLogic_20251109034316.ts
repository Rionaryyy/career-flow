...
if (phase2.callPlanType && phase2.callPlanType.length > 0) {
    ...
    const newMatches = filtered.filter((p) =>
      p.callOptions?.some((opt) => {
        const optId = String(opt.id)
          .replace(/[\[\]"]/g, "") // 🟩 クォート・ブラケット除去
          .trim();
        if (opt.type === "unlimited") return true;
        if (opt.type !== "time") return false;
        const optLimit = map[optId as keyof typeof map] ?? 0;
        console.log("🧮 CheckTime:", { planId: p.planId, optId, optLimit, limit });
  ...
  if (selectedTypes.includes("monthlyLimit")) {
    ...
    const map = { "monthly30min": 30, "monthly60min": 60, "monthly100min": 100, unlimited: Infinity };
    const limit = map[pref as keyof typeof map] ?? 0;
    ...
    const newMatches = filtered.filter((p) =>
      p.callOptions?.some((opt) => {
        ...
        const optLimit = map[optId as keyof typeof map] ?? 0;
        console.log("🧮 CheckMonthly:", { planId: p.planId, optId, optLimit, limit });
        return optLimit >= limit;
      })
    );
    matches.push(...newMatches);
