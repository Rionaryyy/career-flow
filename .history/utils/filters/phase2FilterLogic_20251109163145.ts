...
if (phase2.needInternationalCallUnlimited === "yes") {
  ...
  filtered = filtered.filter((plan) => {
    // === ① 海外通話プラン対応チェック ===
    const supportsIntl =
      (plan as any).internationalCallOptions === true || // ← JSON上のキー対応
      (plan as any).internationalOptions === true; // ← Plan型上のキー対応

    if (!supportsIntl) return false;

    // === ② キャリア未指定なら全対応プランを残す ===
    if (selectedCarriers.length === 0) {
      console.log(`✅ [Include] ${plan.planId} (キャリア指定なし)`);
      return true;
    }

    // === ③ 選択キャリア（au / rakuten）を含むプランのみ残す ===
    const carrierName = plan.carrier?.toLowerCase() ?? "";
    const match = selectedCarriers.some((c: string) => carrierName.includes(c));
