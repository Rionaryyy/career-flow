...
if (answers.dataUsage) {
  ...
  const usage = answers.dataUsage as keyof typeof map;
  const minRequired = map[usage] ?? 0;

  if (minRequired === Infinity) {
    // ✅ 無制限指定時: 無制限プランのみ残す
    filtered = filtered.filter((plan) => {
      const cap = plan.maxDataGB ?? 0;
      const type = plan.dataLimitType?.toLowerCase() ?? "";
      return (
        cap >= 999 ||               // 例: 1000GB以上で実質無制限
        cap === Infinity ||         // Infinity 明示指定
        type === "unlimited" ||     // データ種別が unlimited
        /無制限|制限なし/.test(plan.planName) // 名前に「無制限」が含まれる
      );
    });
  } else {
    // ✅ GB数指定時: その容量以上のプランを残す
    filtered = filtered.filter((plan) => (plan.maxDataGB ?? 0) >= minRequired);
  }
  ...
  console.log("🟦 データ容量フィルター:", {
    ...
    matched: filtered.map((p) => ({
      carrier: p.carrier,
      maxDataGB: p.maxDataGB,
      dataLimitType: p.dataLimitType,
      planName: p.planName,
    })),
  });
}
