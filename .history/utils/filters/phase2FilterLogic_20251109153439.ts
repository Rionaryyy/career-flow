    ...
    const newMatches = filtered.filter((p) => {
      ...
      if (opts.length) {
        console.log(
          "🧩 [MonthlyCheck]",
          p.planId,
          opts.map((o) => ({ id: o.id, type: o.type }))
        );
      }

      // monthly / unlimited のみ対象
      const monthlyOpts = opts.filter(
        (o) => o.type === "monthly" || o.type === "unlimited"
      );

      const hit = monthlyOpts.some((opt) => {
        const optLimit = map[opt.id] ?? 0;
        const result = optLimit >= limit || opt.type === "unlimited";
        if (result) {
          console.log(
            `✅ [MonthlyHit] ${p.planId} → opt.id=${opt.id}, optLimit=${optLimit}, limit=${limit}`
          );
        }
        return result;
      });
