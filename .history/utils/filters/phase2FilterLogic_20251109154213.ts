...
if (selectedTypes.includes("monthlyLimit")) {
  ...
  if (!pref) {
    ...
    const newMatches = filtered.filter((p) => {
      const hasUnlimitedOption = hasUnlimited(p); // ✅ 共通ヘルパーを使用
      const monthlyOpts =
        p.callOptions?.filter((opt) => opt.type === "monthly") ?? [];
      ...
      const hasHigherTier = monthlyOpts.some((opt) => {
        ...
        if (result)
          console.log(
            `✅ [MonthlyHit] ${p.planId} → ${opt.id} (${optLimit}分) >= ${limit}`
          );
        return result;
      });

      if (hasUnlimitedOption)
