...
export function filterPlansByPhase2(answers: DiagnosisAnswers, plans: Plan[]): Plan[] {
  ...
  if (answers.needInternationalCallUnlimited === "yes") {
    const selectedCarriers = Array.isArray(answers.internationalCallCarrier)
      ? answers.internationalCallCarrier.map((c) => c.toLowerCase())
      : [];
    ...
    filtered = filtered.filter((plan) => {
      ...
      // 対応キャリアを限定（楽天 or au）
      const isSupportedCarrier = ["rakuten", "au"].some((key) => carrier.includes(key));

      if (selectedCarriers.length > 0) {
        const isSelectedCarrier = selectedCarriers.some((key) => carrier.includes(key));
        return isSupportedCarrier && isSelectedCarrier;
      }

      return isSupportedCarrier;
    });
  }
