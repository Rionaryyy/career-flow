...
// ===================================================
// 📱 端末フィルター（全シリーズ対応: iPhone / Galaxy / Pixel / Xperia）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      // ✅ スペースを許容してシリーズ抽出（Galaxy S25 / GalaxyS25 両対応）
      const baseSeries =
        normalizedModel.match(/(iphone|galaxy|xperia|(google)?pixel)\s*\d+/)?.[0]
          ?.replace(/\s+/g, "") ?? normalizedModel;

      // ✅ FE / Ultra / Plus / Edge / Fold / A など許可
      const seriesRegex = new RegExp(
        `^(google)?${baseSeries}(pro|plus|ultra|fold|fe|edge|a|mini)?$`
      );
      ...
      filtered = filtered.filter((plan) => {
        ...
        if (match)
          console.log("✅ Hit:", plan.carrier, plan.planName, plan.deviceModel);
        return match;
      });
