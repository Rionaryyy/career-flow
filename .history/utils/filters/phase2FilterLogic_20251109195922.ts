...
// ===================================================
// 📱 端末フィルター（Galaxy FE / Ultra / Edge 完全対応版）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      // ✅ スペースありも考慮したシリーズ抽出（Galaxy S25 / GalaxyS25 両対応）
      const baseSeries =
        normalizedModel.match(/(iphone|galaxy|xperia|(google)?pixel)\s*\d+/)?.[0]
          ?.replace(/\s+/g, "") ?? normalizedModel;

      // ✅ スペース無視で FE / Ultra / Edge / Fold などを許可
      const seriesRegex = new RegExp(
        `^(google)?${baseSeries}(\\s*|\\s*-)?(pro|plus|ultra|fold|fe|edge|a|mini)?$`
      );
      ...
      filtered = filtered.filter((plan) => {
        ...
        const match = models.some((m) => {
          const ok = seriesRegex.test(m);
          if (ok) console.log("✅ Hit:", plan.planName, "→", m);
          return ok;
        });
