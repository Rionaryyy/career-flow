...
// ===================================================
// 📱 端末フィルター（端末存在判定＋「その他」スルー最終版）
// ---------------------------------------------------
// deviceModel が "iPhone 17, iPhone 17 Pro, ..." のように
// 全モデル名をカンマ区切りで持っている前提。
// 「その他」が選ばれた場合はフィルタをスキップ。
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  if (
    ...
    console.log("🟢 [DeviceFilter] その他が選択されたためフィルタをスキップ");
  }

  // === 1️⃣ 正規店購入 ===
  else if (purchaseMethod === "store_purchase") {
  ...
  // === 2️⃣ キャリア購入 or 返却プログラム ===
    ...
    filtered = filtered.filter((plan) => {
      ...
      // ✅ 実在モデル完全一致チェック
      const matched = models.some((m) => m === normalizedModel);

      if (matched)
        console.log(`✅ [DeviceFilter] ${selectedModel} found in ${plan.planName}`);
