...
if (selectedTypes.includes("timeLimit")) {
  ...
  const newMatches = filtered.filter((p) => {
    if (p.callOptions?.length) {
      // 🟩 ここで opt.type を直接ログ出力
      console.log("🧩 [Check type]", p.planId, p.callOptions.map(o => o.type));
      console.log("🧩 [callOptions debug]", p.planId, p.callOptions);

    }

    return p.callOptions?.some((opt) => {
  ...
  if (selectedTypes.includes("monthlyLimit")) {
    ...
    if (!pref) {
      console.log("⏸️ [MonthlyLimit] 月間指定なしのためスキップ");
    } else {
      const map: Record<string, number> = {
        "monthly30min": 30,
        "monthly60min": 60,
      ...
      const newMatches = filtered.filter((p) =>
        p.callOptions?.some((opt) => {
          console.log("🧩 [callOptions debug-monthly]", p.planId, p.callOptions);
  ...
  if (selectedTypes.includes("hybrid")) {
    ...
    if (!pref) {
      console.log("⏸️ [Hybrid] 指定なしのためスキップ");
    } else {
      const map = {
      ...
      const newMatches = filtered.filter((p) =>
        p.callOptions?.some((opt) => {
          const optId = String(opt.id).trim();
          const def = map[optId as keyof typeof map] ?? { count: 0, perCall: 0 };
          return (
            (opt.type === "hybrid" &&
              def.count >= count &&
              def.perCall >= perCall) ||
