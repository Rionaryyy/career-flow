...
if (phase2.devicePreference === "yes") {
  ...
  // ✅ 「その他」選択時はフィルタもreturnも完全スキップ（他のロジックが上書きしても防ぐ）
  if (
    !selectedModel ||
    selectedModel === "その他" ||
    selectedModel.toLowerCase().includes("other")
  ) {
    console.log("🟢 [DeviceFilter] その他が選択されたため全フィルタ処理をスキップ（全通過）");
    console.log("📊 [Device Filter Result] 絞り込み後:", plans.length);
    return plans; // ← 🔥 filteredではなくplansをそのまま返す（これが決め手）
  }
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    filtered = filtered.filter((plan) => {
      ...
      return models.some((m) => m === normalizedModel);
    });
  }
