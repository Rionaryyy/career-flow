...
if (selectedTypes.includes("monthlyLimit")) {
  ...
  if (!pref) {
    ...
    const newMatches = filtered.filter((p) => {
      ...
      const hasMonthly = opts.some((opt) => {
        const optId = String(opt.id).trim();
        const optLimit = map[optId] ?? 0;
        return opt.type === "monthly" && optLimit >= limit;
      });

      // 💡 無制限を別判定として分離
      const hasUnlimited = opts.some((opt) => opt.type === "unlimited");

      const result = hasMonthly || hasUnlimited;
      if (result) {
        console.log(
          `✅ Hit: ${p.planId} (${opts.map((o) => `${o.id}:${o.type}`).join(", ")})`
        );
      }
      return result;
    });
