    ...
    const newMatches = filtered.filter((p) => {
      ...
      if (opts.length) {
        console.log(
          "🧩 [Check]",
          p.planId,
          opts.map((o) => ({ id: o.id, type: o.type, fee: o.fee }))
        );
      }

      const hasUnlimitedOption = opts.some((o) => o.type === "unlimited");
      const monthlyLimits =
        opts
      ...
      if (hasUnlimitedOption || hasHigherTier) {
        console.log("✅ [Hit]", p.planId, monthlyLimits, ">= limit", limit);
      }
