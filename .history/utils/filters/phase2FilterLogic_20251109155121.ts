...
// 🟩 callPlanType が未選択、または "noPreference" のみならスキップ
if (
  !phase2.callPlanType ||
  ...
  // ... 以降の timeLimit / monthlyLimit / hybrid / unlimited 分岐は現行のままでOK
}

if (!phase2.callPlanType || phase2.callPlanType.length === 0) {
  console.log("💤 [CallFilter] 通話タイプ未選択のためスキップ");
} else {
  const selectedTypes = phase2.callPlanType;
  let matches: Plan[] = [];
  ...
  if (selectedTypes.includes("monthlyLimit")) {
    ...
    if (!pref) {
      ...
      const newMatches = filtered.filter((p) => {
        const hasUnlimitedOption = hasUnlimited(p);
        const monthlyOpts = p.callOptions?.filter(
          (opt) => opt.type === "monthly"
        ) ?? [];
  ...
  if (selectedTypes.includes("hybrid")) {
    ...
    if (!pref) {
      ...
      const newMatches = filtered.filter((p) => {
        const hasUnlimitedOption = hasUnlimited(p);
        const hybridOptions = p.callOptions?.filter(
          (opt) => opt.type === "hybrid"
        ) ?? [];
