...
if (selectedTypes.includes("monthlyLimit")) {
  ...
  if (!pref) {
    ...
    const newMatches = filtered.filter((p) => {
      const hasUnlimited = p.callOptions?.some(opt => opt.type === "unlimited");
      const monthlyList = p.callOptions
        ?.filter(opt => opt.type === "monthly")
        .map(opt => opt.id) ?? [];

      // 🔍 デバッグ出力
      if (p.callOptions?.length) {
        console.log(`🧩 [MonthlyCheck] ${p.planId}`, p.callOptions.map(opt => `${opt.id}:${opt.type}`));
      }

      const hasMatch = p.callOptions?.some((opt) => {
