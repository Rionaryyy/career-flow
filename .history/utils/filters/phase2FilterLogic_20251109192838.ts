...
if (phase2.devicePreference === "yes") {
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      // ✅ シリーズ名（例: "iphone17"）だけを抽出
      const baseSeries = normalizedModel.match(/(iphone|galaxy|pixel|xperia)\d+/)?.[0] ?? normalizedModel;
      ...
      filtered = filtered.filter((plan) => {
        ...
        // ✅ 同一シリーズ内で部分一致（例: iPhone17 ⇔ iPhone17Pro）
        // ただし iPhone16 は除外される
        const match = models.some((m) =>
          new RegExp(`^${baseSeries}([a-z]*|pro|plus|ultra)?$`).test(m)
        );
