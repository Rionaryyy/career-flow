...
if (phase2.devicePreference === "yes") {
  ...
  // === キャリア購入 or 返却プログラム ===
    ...
    if (selectedModel) {
      // 🧩 選択モデル正規化（例: "iPhone 17 Pro" → "iphone17pro"）
      const normalizedModel = selectedModel
        .toLowerCase()
        .replace(/\s+/g, "")
        .replace(/[（）]/g, ""); // 全角カッコ除去

      // 🧩 シリーズ抽出（pro/max/plus/ultra/fe/eなどを削除）
      const baseSeries = normalizedModel
        .replace(/pro|max|plus|ultra|mini|fe|e/g, "")
        .trim();

      console.log("🧩 [Device Filter Debug]", {
        selectedModel,
        normalizedModel,
        baseSeries,
      });
      ...
      filtered = filtered.filter((plan) => {
        ...
        const match = models.some((m: string) => m.includes(baseSeries));

        if (match) {
          console.log("✅ Hit:", plan.carrier, plan.planName, models);
        }
