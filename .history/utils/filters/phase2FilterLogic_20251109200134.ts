...
// ===================================================
// 📱 端末フィルター（Galaxy S25 FE / Ultra / Edge / Plus 対応 完全版）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      // ✅ 数字までのシリーズ名を抽出（例: Galaxy S25 FE → galaxys25）
      const baseSeries =
      ...
      filtered = filtered.filter((plan) => {
        ...
        // ✅ 「含む」方向に変更 → FE/Ultraなども拾う
        const matchedModel = models.find((m) => m.includes(baseSeries));

        if (matchedModel) {
          console.log("✅ Hit:", plan.planName, "→", matchedModel);
          return true;
        }
        return false;
      });
