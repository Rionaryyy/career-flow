...
if (phase2.devicePreference === "yes") {
  ...
  // === キャリア購入・返却プログラムの場合 ===
    ...
    if (selectedModel) {
      ...
      // 🧩 「シリーズ名（例: iPhone17）」を抽出
      const baseSeries = normalizedModel
        .replace(/pro|max|plus|ultra|mini|e|fe/g, "") // 派生語を除外
        .trim();

      filtered = filtered.filter((plan) => {
        const modelStr = plan.deviceModel ?? "";
        const models = modelStr
          .split(",")
          .map((m: string) => m.toLowerCase().trim().replace(/\s+/g, ""));

        // ✅ baseSeries（例: iphone17）を含むプランを残す
        return models.some((m: string) => m.includes(baseSeries));
      });
    }
  }
}
