    ...
    const newMatches = filtered.filter((p) => {
      ...
      if (opts.length) {
        console.log(
          "🧩 [MonthlyCheck]",
          p.planId,
          opts.map((o) => `${o.id}(${o.type})`)
        );
      }

      // === monthly または unlimited のみチェック ===
      const hit = opts.some((opt) => {
        if (!opt || !opt.type) return false;
        const optId = String(opt.id).trim();
        const optType = String(opt.type).trim();

        // type: monthly の場合は、idで比較してmapを参照
        if (optType === "monthly") {
          const optLimit = map[optId] ?? 0;
          const result = optLimit >= limit;
          if (result) {
            console.log(
              `✅ [MonthlyHit] ${p.planId} → ${optId} (${optLimit}分) >= ${limit}`
            );
          }
          return result;
        }

        // type: unlimited の場合は常に通す
        if (optType === "unlimited") {
          console.log(`💬 [UnlimitedPass] ${p.planId} → ${optId}`);
          return true;
        }
