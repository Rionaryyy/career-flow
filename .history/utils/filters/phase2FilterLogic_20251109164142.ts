...
if (phase2.callOptionsNeeded === "yes") {
  ...
  const beforeSamples = filtered.slice(0, 5).map((p) => ({
    planId: p.planId,
    hasVoicemail: (p as any).hasVoicemail,
  }));
  console.log("🧩 [BeforeFilter Sample]:", beforeSamples);

  filtered = filtered.filter((plan) => {
    const hasVoicemailFlag =
      (plan as any).hasVoicemail === true ||
      (plan as any)["hasVoicemail,callOptionsNeeded"] === true; // ← データ誤結合対応
    if (hasVoicemailFlag) {
      console.log(`✅ [Include] ${plan.planId} (hasVoicemail: true)`);
    } else {
      console.log(`❌ [Exclude] ${plan.planId} (hasVoicemail: ${plan.hasVoicemail})`);
    }
    return hasVoicemailFlag;
  });

  console.log(`📉 [VoicemailFilter] ${before} → ${filtered.length} 件`);
}
