...
// ===================================================
// 📞 国内通話プランフィルター（UI質問準拠・無制限選択時はunlimited含む全プラン残す版）
// ===================================================
if (phase2.callPlanType && phase2.callPlanType.length > 0) {
  ...
  const hasUnlimited = (p: Plan): boolean => {
    ...
    const inAvailableList = p.availableCallOptions?.includes("unlimited");
  ...
  if (selectedTypes.includes("timeLimit")) {
    ...
    } else if (pref === "unlimited") {
      console.log("💬 [TimeLimit] 無制限選択 → 無制限プランのみ残す");
  ...
  if (selectedTypes.includes("monthlyLimit")) {
    ...
    if (!pref) {
      ...
      console.log("💬 [MonthlyLimit] 無制限選択 → 無制限プランのみ残す");
      const newMatches = filtered.filter(hasUnlimited);
      matches.push(...newMatches);
    } else {
      const map: Record<string, number> = {
  ...
  if (selectedTypes.includes("hybrid")) {
    ...
    if (!pref) {
      ...
      console.log("💬 [Hybrid] 無制限選択 → 無制限プランのみ残す");
      const newMatches = filtered.filter(hasUnlimited);
      matches.push(...newMatches);
    } else {
      const map = {
