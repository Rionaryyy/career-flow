...
// 📞 留守番電話オプションフィルター（安定版）
// ===================================================
if (phase2.callOptionsNeeded) {
  const beforeCount = filtered.length;
  const needVoicemail = phase2.callOptionsNeeded === "yes";

  console.log(`📞 [VoicemailFilter] 回答: ${phase2.callOptionsNeeded}`);

  filtered = filtered.filter((plan) => {
    const hasFlag =
      plan.hasVoicemail === true ||
      (plan as any)["hasVoicemail,callOptionsNeeded"] === true;

    // yes の場合 → hasVoicemail=true のみ残す
    // no の場合 → hasVoicemail=false または undefined のみ残す
    return needVoicemail ? hasFlag : !hasFlag;
  });

  console.log(
    `📉 [VoicemailFilter] ${beforeCount} → ${filtered.length} 件 (${needVoicemail ? "必要" : "不要"})`
  );

  // 🔍 デバッグログ（どのプランが残ったか）
  filtered.forEach((p) => {
    console.log(
      `   ┗ ${p.planId}: hasVoicemail=${p.hasVoicemail}`
    );
  });
}
