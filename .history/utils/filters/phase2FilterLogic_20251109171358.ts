...
if (phase2.overseasSupport === "yes") {
  ...
  filtered = filtered.filter((plan) => {
    // 🧩 値をすべて文字列化して確認
    const rawValue = String((plan as any).overseasSupport);
    const intlOpt = (plan as any).internationalOptions;
    const intlCall = (plan as any).internationalCallOptions;

    const isSupported =
      rawValue === "true" || // ← JSONで文字列trueでも通す
      rawValue === "1" ||
      (plan as any).overseasSupport === true ||
      intlOpt === true ||
      intlCall === true;

    if (isSupported) {
      console.log(`✅ [OverseasKeep] ${plan.planId} (${plan.carrier}) overseasSupport=${rawValue}`);
    } else {
      console.log(`❌ [OverseasDrop] ${plan.planId} (${plan.carrier}) overseasSupport=${rawValue}`);
    }
