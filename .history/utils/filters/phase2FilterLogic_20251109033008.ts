...
if (phase2.callPlanType && phase2.callPlanType.length > 0) {
  ...
  if (selectedTypes.includes("timeLimit")) {
    ...
    const newMatches = filtered.filter((p) =>
      p.callOptions?.some((opt) => {
        if (opt.type === "unlimited") return true;
        if (opt.type !== "time") return false;
        const optLimit = map[opt.id as keyof typeof map] ?? 0;
        return optLimit >= limit;
      })
    );

    matches.push(...newMatches);
    console.log(`📉 時間制限型フィルター: ${before} → ${matches.length} 件`);
  }

  // === 2. 月間制限型 ===
  if (selectedTypes.includes("monthlyLimit")) {
    const pref = phase2.monthlyLimitPreference ?? "";
    const map = { "monthly30min": 30, "monthly60min": 60, "monthly70min": 70, "monthly100min": 100, unlimited: Infinity };
    const limit = map[pref as keyof typeof map] ?? 0;
    ...
    const newMatches = filtered.filter((p) =>
      p.callOptions?.some((opt) => {
        if (opt.type === "unlimited") return true;
        if (opt.type !== "monthly") return false;
        const optLimit = map[opt.id as keyof typeof map] ?? 0;
        return optLimit >= limit;
      })
    );
    ...
    matches.push(...newMatches);
    console.log(`📉 月間制限型フィルター: ${before} → ${matches.length} 件`);
  }
  ...
  if (selectedTypes.includes("hybrid")) {
    ...
    const map = {
      ...
      "5minX30calls": { count: 30, perCall: 5 },
    ...
    const newMatches = filtered.filter((p) =>
      p.callOptions?.some((opt) => {
        if (opt.type === "unlimited") return true;
        if (opt.type !== "hybrid") return false;
        const target = map[opt.id as keyof typeof map];
        return target && target.count >= count && target.perCall >= perCall;
      })
    );

    matches.push(...newMatches);
    console.log(`📉 ハイブリッド型フィルター: ${before} → ${matches.length} 件`);
  }
