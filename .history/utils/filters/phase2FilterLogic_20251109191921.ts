  ...
  if (purchaseMethod === "store_purchase") {
    ...
    if (selectedModel) {
      ...
      const normalizedModel = selectedModel
        ...
        .replace(/\s+/g, "") // 空白除去
        .replace(/[（）]/g, "") // 全角カッコ除去
        .replace(/pro|max|plus|ultra|mini|fe|e/g, ""); // 派生語削除
      ...
      filtered = filtered.filter((plan) => {
        ...
        // ✅ 部分一致（"iphone17" → "iphone17pro"もヒット）
        const match = models.some((m) => m.includes(normalizedModel));
        if (match) {
