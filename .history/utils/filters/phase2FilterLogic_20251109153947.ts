...
if (selectedTypes.includes("monthlyLimit")) {
  ...
  if (!pref) {
    ...
    const newMatches = filtered.filter((p) => {
      const opts = p.callOptions ?? [];

      const hasUnlimited = opts.some(
        (opt) => opt.id === "unlimited" || opt.type === "unlimited"
      );

      const monthlyOpts = opts.filter(
        (opt) => opt.type === "monthly" && !!map[opt.id]
      );

      const hit = hasUnlimited || monthlyOpts.some((opt) => {
        const optLimit = map[opt.id] ?? 0;
        const result = optLimit >= limit;
        if (result) {
          console.log(`✅ [MonthlyHit] ${p.planId} → ${opt.id} (${optLimit}分) >= ${limit}`);
        }
        return result;
      });

      if (hasUnlimited) {
        console.log(`💬 [UnlimitedPass] ${p.planId} → 無制限`);
      }
