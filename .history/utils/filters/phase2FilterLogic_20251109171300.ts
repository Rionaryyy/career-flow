...
if (phase2.overseasSupport === "yes") {
  ...
  filtered = filtered.filter((plan) => {
    const flag =
      plan.overseasSupport === true ||
      (plan as any).overseasSupport === "true" ||
      (plan as any)["overseasSupport "] === true || // ← スペース誤混入にも対応
      (plan as any)["海外対応"] === true || // ← CSVなどから読み込んだときの和名にも対応
      (plan as any).internationalOptions === true ||
      (plan as any).internationalCallOptions === true;

    if (flag) {
      console.log(`✅ [OverseasKeep] ${plan.planId} (${plan.carrier})`);
    } else {
      console.log(`❌ [OverseasDrop] ${plan.planId} (${plan.carrier})`);
    }
    return flag;
  });
