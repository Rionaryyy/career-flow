...
// ===================================================
// 🌏 海外利用フィルター（overseasSupport + internationalOptions対応）
// ===================================================
if (phase2.overseasSupport === "yes") {
  ...
  filtered = filtered.filter((plan) => {
    ...
    if (supportFlag) {
      console.log(`✅ [OverseasInclude] ${plan.planId} (${plan.carrier})`);
    } else {
      console.log(`❌ [OverseasExclude] ${plan.planId} (${plan.carrier})`);
    }
    return supportFlag;
  });
