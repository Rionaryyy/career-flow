...
// ===================================================
// 📱 端末フィルター（「いいえ」リセット／「その他」全通過／一致型フィルタ完全版）
// ===================================================
if (phase2.devicePreference === "yes") {
  ...
  // === 2️⃣ キャリア購入 or 返却（指定モデルを含むプランのみ残す）
    ...
    filtered = filtered.filter((plan) => {
      ...
      // ✅ 部分一致許可
      return models.some(
        (m) => m.includes(normalizedModel) || normalizedModel.includes(m)
      );
    });
  }
...
// === 「いいえ」選択時 → device関連フィルタ全スキップ（リセット扱い） ===
else if (phase2.devicePreference === "no") {
  console.log("🧹 [DeviceFilter] 端末購入なし選択 → device関連条件をリセット（全プラン通過）");
  // ✅ ここでは devicePurchaseRequired, simOnlyAvailable などを一切触らない
  //    → つまり filtered = filtered（現状維持＝全件通過）
}


// ===================================================
// 🌏 海外利用フィルター（型ズレ・欠落完全防御＋文字揺れ対応）
// ===================================================

console.log("🌍 OverseasSupport debug sample:",
  filtered.slice(0, 5).map(p => ({
    id: p.planId,
    carrier: p.carrier,
    overseasSupport: p.overseasSupport
  }))
);

// === 小文字化して比較 ===
const overseasValue = (phase2.overseasSupport ?? "").toString().toLowerCase();
...
if (["yes", "はい", "あり", "有", "true"].includes(overseasValue)) {
  ...
  filtered = filtered.filter((plan) => {
    ...
    const flag = val === true || val === "true" || val === 1;

    if (flag) {
      console.log(`✅ [OverseasKeep] ${plan.planId} (${plan.carrier}) val=${val}`);
    } else {
      console.log(`❌ [OverseasDrop] ${plan.planId} (${plan.carrier}) val=${val}`);
    }
  ...
  console.log("💤 [OverseasFilter] 海外利用条件に該当せずスキップ:", phase2.overseasSupport);
}
