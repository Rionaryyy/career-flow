...
try {
  ...
  (Array.isArray(answers.phase2?.internationalCallCarrier) &&
    answers.phase2!.internationalCallCarrier.length > 0);


  if (wantsIntlCall) {
    const selected = Array.isArray(answers.phase2?.internationalCallCarrier)
      ? answers.phase2.internationalCallCarrier
      : [];
    ...
    for (const c of selected) {
      ...
      if (carrierMatch) {
        ...
        if (intlOption && typeof intlOption.fee === "number") {
          ...
          console.log(`⚠️ ${plan.carrier}: 国際通話オプション未定義またはfee不明`);
        }
      }
    }
  }
...
console.log("🧭 reached before Subscription block", plan.carrier);
...
console.log("🧠 Subscription keys:", Object.keys(subsSource));
...
if (allSubs.length === 0) {
  ...
  allSubs.forEach((subName, i) => {
    ...
    console.log(`🎯 Matched entries for ${subName}:`, matchedEntries.map((e) => e.key));
    ...
    matchedEntries.forEach((entry) => {
      ...
      if (entry.key.endsWith("_reward")) {
        ...
        if (!isNaN(rate) && rate > 0) {
          ...
          console.log(`⚠️ 還元スキップ: ${plan.carrier} - ${entry.name} (rate=${entry.rewards?.[carrierKey]})`);
        }
      }
    });
  });
