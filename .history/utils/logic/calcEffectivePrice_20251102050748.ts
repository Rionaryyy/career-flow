...
const rewardEntry = subscriptionData.find((s) => {
  ...
  return (
    ...
    (sub.includes(target) || target.includes(sub))
  );
});
// 👇このブロックをここに追加！
if (rewardEntry) {
  console.log("🎯 rewardEntry FOUND:", rewardEntry.name, rewardEntry.rewards[carrierKey]);
} else {
  console.log("❌ rewardEntry NOT FOUND:", subName);
}
if (rewardEntry && rewardEntry.rewards[carrierKey] !== undefined) {  // ←ここ変更
