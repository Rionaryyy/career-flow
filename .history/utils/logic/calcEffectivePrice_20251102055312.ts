...
export function calculatePlanCost(plan: Plan, answers: DiagnosisAnswers): PlanCostBreakdown {
  console.log("🚀 calculatePlanCost called for", plan.carrier);
    ...
    console.log("🧩 calculatePlanCost START reached:", plan.carrier);
  console.log("🧩 typeof answers.phase2:", typeof answers.phase2, answers.phase2);
    // 🧠 安全に phase2 を展開する（answers 直下にサブスク項目があるケースも想定）
  const phase2 = answers.phase2 ?? (answers as any);
  console.log("🧠 phase2 resolved for carrier:", plan.carrier, phase2);
...
console.log("🎬 Subscription block START", {
  video: phase2?.videoSubscriptions,
  music: phase2?.musicSubscriptions,
...
const allSubs = [
  phase2?.subscriptionList ?? [],
  phase2?.videoSubscriptions ?? [],
  phase2?.musicSubscriptions ?? [],
  phase2?.bookSubscriptions ?? [],
  phase2?.gameSubscriptions ?? [],
  phase2?.cloudSubscriptions ?? [],
  phase2?.otherSubscriptions ?? [],
]
...
if (allSubs.length) {
  ...
  allSubs.forEach((subName, i) => {
    ...
    const matchedEntries = subscriptionData.filter((s) => {
      ...
      const match =
        normalizedSub.includes(target) ||
        target.includes(normalizedSub);
      if (match)
    ...
    matchedEntries.forEach((entry) => {
      ...
      if (entry.key.endsWith("_set") && entry.discounts[carrierKey]) {
        const discount = entry.discounts[carrierKey] ?? 0;
      ...
      if (entry.key.endsWith("_reward") && entry.rewards[carrierKey]) {
        const rate = entry.rewards[carrierKey] ?? 0;
