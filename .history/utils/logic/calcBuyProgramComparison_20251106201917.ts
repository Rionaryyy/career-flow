...
export function compareBuyPrograms(
  ...
  // 指定モデル＋容量に一致する購入プランを抽出（部分一致対応に修正）
  ...
  const targets = deviceList.filter((d) => {
    const matchModel = normalize(d.model).includes(normalize(model));
    const matchStorage = normalize(d.storage).includes(normalize(storage));
    return (
      matchModel &&
      matchStorage &&
