...
// === 💡 「キャリアセット割を希望するか？」をチェック ===
const wantsCarrierSubs =
  typeof subsSource.subscriptionMonthly === "string" &&
  subsSource.subscriptionMonthly.includes("はい");

// サブスク割引・還元の初期値を確保
let subscriptionDiscount = 0;
let subscriptionReward = 0;

// 🚫 ユーザーがセット割を希望しない場合はスキップ
if (!wantsCarrierSubs) {
  console.log(`🚫 サブスク割スキップ: ${plan.carrier} - ユーザーがセット割希望していません`);
} else {
