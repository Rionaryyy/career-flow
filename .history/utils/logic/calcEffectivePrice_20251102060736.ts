...
allSubs.forEach((subName, i) => {
  ...
  const matchedEntries = subscriptionData.filter((s) => {
    ...
    // カタカナ略称も英語化（ネトフリ/アマプラ等対応）
    const normalized = normalizedSub
      .replace(/ネトフリ/g, "netflix")
      .replace(/アマプラ/g, "amazonprimevideo")
      .replace(/スポティファイ/g, "spotify")
      .replace(/アップルミュージック/g, "applemusic");
  ...
  matchedEntries.forEach((entry) => {
    ...
    if (entry.key.endsWith("_reward")) {
      const rate = Number(entry.rewards?.[carrierKey] ?? 0);
      if (rate > 0) {
        ...
        console.log(`⚠️ 還元データなし: ${plan.carrier} - ${entry.name}`);
      }
    }
  });
});
