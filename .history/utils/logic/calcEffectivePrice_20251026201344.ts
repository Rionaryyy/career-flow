...
if (typeof buyingText === "string" && /(購入|分割|一括)/.test(buyingText)) {
  ...
  const match = devicePricesBuy.find((d) => {
    const modelMatch = d.model?.toLowerCase().includes(selectedModel ?? "");
    const storageMatch = d.storage?.toLowerCase() === selectedStorage;

    // 「正規店購入」の場合はキャリア一致を無視して全候補から探す
    if (!isCarrierPurchase) {
      return modelMatch && storageMatch;
    }

    // キャリアで購入の場合は carrier も一致させる
    return (
      d.ownershipType === "buy" &&
      d.carrier?.toLowerCase() === plan.carrier?.toLowerCase() &&
      modelMatch &&
