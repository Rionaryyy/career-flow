...
if (allSubs.length) {
  ...
  allSubs.forEach((subName) => {
    const normalizedSub = subName.replace(/（.*?）/g, "").toLowerCase();

    // === セット割 ===
    const setEntry = subscriptionData.find(
      (s) =>
        s.key.endsWith("_set") &&
    ...
    // === 本体価格還元 ===
const normalizeSubName = (text: string) =>
  text
    .replace(/（.*?）/g, "") // 括弧削除
    .replace(/[^a-zA-Z0-9ぁ-んァ-ン一-龠]/g, "") // 記号や空白を除去
    .toLowerCase();

const rewardEntry = subscriptionData.find((s) => {
  const cleanKey = normalizeSubName(
    s.name.replace("（セット割）", "").replace("（本体価格還元）", "")
  );
  const cleanInput = normalizeSubName(subName);

  // 部分一致を許可（例：Prime == PrimeVideo）
  return s.key.endsWith("_reward") && cleanInput.includes(cleanKey);
});

    if (rewardEntry && rewardEntry.rewards[carrierKey]) {
