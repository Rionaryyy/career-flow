...
// 💰 ユーザー回答ベースの月額支払い金額
const paymentText = answers.phase2?.paymentMonthly ?? "0";
const paymentAmount = typeof paymentText === "number"
  ? paymentText
  : parseInt(paymentText.toString().replace(/\D/g, ""), 10) || 0;

// 🌟 実際の還元額 = 月間支払額 × 還元率
dailyPaymentReward = Math.round(paymentAmount * dailyPaymentRate);

console.log(
  `🪙 日々の支払い還元: 経済圏=${ecosystem}, rate=${dailyPaymentRate}, base=${paymentAmount}, reward=${dailyPaymentReward}`
);
...
// 💰 ユーザー回答ベースのショッピング金額
const shoppingText = answers.phase2?.shoppingMonthly ?? "0";
const shoppingAmount = typeof shoppingText === "number"
  ? shoppingText
  : parseInt(shoppingText.toString().replace(/\D/g, ""), 10) || 0;

// 🌟 実際の還元額 = ショッピング金額 × 還元率
shoppingReward = Math.round(shoppingAmount * shoppingRate);

console.log(
  `🛒 ショッピング還元: 経済圏=${ecosystem}, rate=${shoppingRate}, base=${shoppingAmount}, reward=${shoppingReward}`
);
