...
let subscriptionBaseFee = 0; // ←★ サブスク本体料金の合計
...
if (allSubs.length === 0) {
  ...
  // === 💙 STEP 1: 全プランに対してサブスク本体料金を加算（付帯除外） ===
  const includedSubs =
  ...
  allSubs.forEach((subName) => {
    ...
    const isIncluded = includedSubs.some((inc) => normalized.includes(inc));
...
if (allSubs.length > 0) {
  ...
  const alreadyAdded = new Set(
    (subscriptionDetails ?? []).map((s) => s.name.toLowerCase())
  );

  console.log("🎬 サブスク加算判定:", {
    carrier: plan.carrier,
    includedSubs,
    selectedSubs: allSubs,
    alreadyAdded: Array.from(alreadyAdded),
  });
  ...
  allSubs.forEach((subName) => {
    ...
    // subscriptionDataからベース料金を取得
    const subEntry = subscriptionData.find((s) =>
      s.name.toLowerCase().includes(normalized)
    );
    const baseFee = subEntry?.basePrice ?? 0;

    // すでに追加済み、または付帯済みならスキップ
    if (alreadyAdded.has(normalized)) {
      console.log(`⏭ ${subName}: すでに追加済み → スキップ`);
      return;
    }

    if (!isIncluded && baseFee > 0) {
      subscriptionBaseFee += baseFee;
      subscriptionDetails.push({
        name: subEntry?.name ?? subName,
        basePrice: baseFee,
        discount: 0,
        reward: 0,
      });
      alreadyAdded.add(normalized);
      console.log(`➕ ${plan.carrier}: ${subName} は付帯なし → +¥${baseFee}`);
    } else if (isIncluded) {
      console.log(`✅ ${plan.carrier}: ${subName} は付帯済み → 加算なし`);
    }
  });
