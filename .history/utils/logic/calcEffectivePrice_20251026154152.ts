...
const buyingDeviceText = answers.phase2?.buyingDevice ?? "";

if (
  typeof buyingDeviceText === "string" &&
  (buyingDeviceText.includes("返却") ||
    buyingDeviceText.includes("プログラム") ||
    buyingDeviceText.includes("リース"))
) {
  const selectedModel = answers.phase2?.deviceModel;
  const selectedStorage = answers.phase2?.deviceStorage;
  ...
  const total =
    ...
    initialFeeMonthly +
    tetheringFee +
    deviceLeaseMonthly; // ✅ ← 最後に加算（端末費を最後に合算）
