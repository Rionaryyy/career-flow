...
// 🧠 Phase1 の回答も確認できるように取得（answers 全体が渡っている前提）
const includeSubs =
  (answers.phase1?.includeSubscription ?? "").includes("はい") ||
  (answers.includeSubscription ?? "").includes("はい"); // fallback for flat answers

// 💡 フェーズ①で「サブスク比較しない」を選んでいたらスキップ
if (!includeSubs) {
  console.log(`🚫 ${plan.carrier}: サブスク比較対象外（Phase1で「いいえ」選択）`);
} else {
  const subsSource = phase2 as any;
