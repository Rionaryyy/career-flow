...
console.log("🎬 Subscription block START", {
  video: answers.phase2?.videoSubscriptions,
  music: answers.phase2?.musicSubscriptions,
...
// 全カテゴリ統合
const allSubs = [
  answers.phase2?.subscriptionList,
...
if (allSubs.length) {
  ...
  const normalize = (text: string) =>
    text
      ...
      .replace(/（.*?）/g, "") // 全角括弧削除
      .replace(/[()]/g, "") // 半角括弧削除
      .replace(/[^a-z0-9ぁ-んァ-ン一-龠]/g, "") // 記号・スペース削除
      .trim();
  ...
  allSubs.forEach((subName) => {
    ...
    // === セット割 ===
    const setEntry = subscriptionData.find((s) => {
      const target = normalize(
        s.name.replace("（セット割）", "").replace("（本体価格還元）", "")
      );
      return s.key.endsWith("_set") && normalizedSub.includes(target);
    });

    if (setEntry && setEntry.discounts[carrierKey]) {
      const discount = setEntry.discounts[carrierKey] ?? 0;
      subscriptionDiscount += discount;
      console.log(`🎬 セット割: ${plan.carrier} - ${setEntry.name} (-¥${discount}/月)`);
    }

// === 本体価格還元 ===
const rewardEntry = subscriptionData.find((s) => {
  const target = normalize(
    s.name
      .replace("（セット割）", "")
      .replace("（本体価格還元）", "")
  );
  const sub = normalize(subName)
    .replace(/video|music|tv|plus|サービス|premium/g, ""); // ←汎用語除外

  return (
    s.key.endsWith("_reward") &&
    (
      sub.includes(target) ||
      target.includes(sub) ||
      sub.startsWith(target) ||
      target.startsWith(sub)
    )
  );
});
...
// デバッグログ強化
if (rewardEntry) {
  console.log("🎯 rewardEntry FOUND:", rewardEntry.name, "→ rate:", rewardEntry.rewards[carrierKey]);
} else {
  console.log("❌ rewardEntry NOT FOUND:", subName, "(normalized:", normalize(subName), ")");
}

if (rewardEntry && rewardEntry.rewards[carrierKey] !== undefined) {
  const base = rewardEntry.basePrice ?? 0;
  const rate = rewardEntry.rewards[carrierKey] ?? 0;
  const reward = Math.round(base * rate);
  subscriptionReward += reward;
  console.log(`💸 還元: ${plan.carrier} - ${rewardEntry.name} (${rate * 100}% → ¥${reward}/月)`);
}
...
subscriptionReward = Math.round(subscriptionReward);
console.log("✅ サブスク割合計:", {
  carrier: plan.carrier,
