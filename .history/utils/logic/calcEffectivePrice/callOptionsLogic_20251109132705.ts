...
export function calcCallOptions(plan: Plan, answers: DiagnosisAnswers): CallOptionResult {
  ...
  if (plan.callOptions?.length) {
    const callOptionMap: Record<string, string> = {
      ...
      "10分以内": "10min",
      "15分以内": "15min",
      "月30分まで無料": "monthly30min",
      "月60分まで無料": "monthly60min",
      "月100分まで無料": "monthly100min",
      "月30回まで各10分無料": "hybrid_30x10min",
      "月50回まで各10分無料": "hybrid_50x10min",
      "無制限（完全定額）": "unlimited",
      ...
      "30min": "monthly30min",
      "60min": "monthly60min",
      "100min": "monthly100min",
      "30x10": "hybrid_30x10min",
      "50x10": "hybrid_50x10min",
      "unlimited": "unlimited",
    };
    ...
    const validOptions = plan.callOptions.filter((opt) => {
      ...
      // 下位互換含めた柔軟一致
      if (matchedIds.includes("5min") && ["10min", "monthly30min", "monthly60min", "unlimited"].includes(opt.id)) return true;
      if (matchedIds.includes("monthly30min") && ["monthly60min", "unlimited"].includes(opt.id)) return true;
      if (matchedIds.includes("hybrid_30x10min") && ["hybrid_50x10min", "unlimited"].includes(opt.id)) return true;
      return false;
    });
  ...
  try {
    ...
    const wantsIntlCall =
      ...
      answers.needInternationalCallUnlimited === "yes" ||
      (Array.isArray(intlCarriers) && intlCarriers.length > 0);

    if (wantsIntlCall) {
      const intlOption = plan.internationalOptions?.find(
        (opt) =>
          (opt.name && /国際通話|international|intl/i.test(opt.name)) ||
          (opt.id && /international|intl/i.test(opt.id))
      );
  ...
  const wantsVoicemail =
    typeof answers.callOptionsNeeded === "string"
      ? answers.callOptionsNeeded.includes("はい") || answers.callOptionsNeeded.includes("yes")
      : false;
  ...
  const wantsTethering =
    (typeof tetheringAnswer === "string" &&
      (tetheringAnswer.includes("はい") || tetheringAnswer.includes("yes"))) ||
