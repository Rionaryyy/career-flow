...
export function calcCampaigns(plan: Plan, answers: DiagnosisAnswers) {
  ...
  // ✅ "store" / "online" / "either" に対応
  if (method.includes("店頭") || method === "store") initialCostTotal = feeStore;
  else if (method.includes("オンライン") || method === "online") initialCostTotal = feeOnline + feeEsim;
  else if (method.includes("どちらでも") || method === "either") initialCostTotal = Math.min(feeStore, feeOnline + feeEsim);
  else initialCostTotal = feeOnline + feeEsim;

  // === 📆 比較期間 ===
  const comparePeriod = answers.comparePeriod ?? answers.phase1?.comparePeriod ?? "";
  let months = 24;
  if (comparePeriod.includes("1年") || comparePeriod.includes("12m")) months = 12;
  else if (comparePeriod.includes("2年") || comparePeriod.includes("24m")) months = 24;
  else if (comparePeriod.includes("3年") || comparePeriod.includes("36m")) months = 36;

  // === 📊 比較軸 ===
  const compareAxis = answers.compareAxis ?? "";
  const considerRealPayment =
    compareAxis.includes("実際に支払う金額") || compareAxis.includes("real_payment");
