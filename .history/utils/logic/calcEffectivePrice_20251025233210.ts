...
// ✅ すべてのサブスクを考慮して複数一致を合算
if (
  answers.phase2?.subscriptionMonthly?.includes("はい") &&
  allSelectedSubscriptions.length > 0 &&
  plan.subscriptionDiscountRules?.length
) {
  const matchedRules = plan.subscriptionDiscountRules.filter((r) =>
    r.applicableSubscriptions?.some((s) =>
      allSelectedSubscriptions.includes(s)
    )
  );

  if (matchedRules.length > 0) {
    // 🎬 合計割引額を合算
    subscriptionDiscount = matchedRules.reduce(
...
const total =
  ...
  electricDiscount -
  gasDiscount -
  subscriptionDiscount - // ← 🎬 修正済：減算扱いに変更
  initialFeeMonthly +
  tetheringFee;
