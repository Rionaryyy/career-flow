...
export function calculatePlanCost(plan: Plan, answers: DiagnosisAnswers): PlanCostBreakdown {
  ...
  // ✅ 修正版（buyingDevice が null の場合、devicePurchaseMethods[0] を使用）
  const buyingText =
  ...
  // 🔍 デバッグポップアップ（購入型条件チェック前）
  if (typeof window !== "undefined") {
    alert(`📱DEBUG\n${JSON.stringify({
      buyingText,
      model: answers.phase2?.deviceModel,
      storage: answers.phase2?.deviceStorage,
      plan: plan.carrier,
    }, null, 2)}`);
  }
  ...
  if (typeof buyingText === "string" && /(購入|分割|一括)/.test(buyingText)) {
    ...
    if (matchBuy) {
      ...
      console.log(`✅ Buy matched: ${matchBuy.model} ${matchBuy.storage} (${matchBuy.carrier}) → ${matchBuy.monthlyPayment}`);
    } else {
