...
if (allSubs.length) {
  ...
  allSubs.forEach((subName) => {
    ...
    if (setEntry && setEntry.discounts[carrierKey]) {
      subscriptionDiscount += setEntry.discounts[carrierKey] ?? 0;
      console.log(
        `🎬 セット割: ${plan.carrier} - ${setEntry.name} (-¥${setEntry.discounts[carrierKey]}/月)`
      );
    }

    // === 本体価格還元（％による割引） ===
    const rewardEntry = subscriptionData.find(
      (s) =>
    ...
    if (rewardEntry && rewardEntry.rewards[carrierKey]) {
      ...
      const reward = base * rate;
      subscriptionReward += reward;
      console.log(
        `💸 還元: ${plan.carrier} - ${rewardEntry.name} (${rate * 100}% → ¥${reward}/月)`
      );
    }
  });
}
...
subscriptionDiscount = Math.round(subscriptionDiscount);
subscriptionReward = Math.round(subscriptionReward);
const totalSubscriptionBenefit = subscriptionDiscount + subscriptionReward;
