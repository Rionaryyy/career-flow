...
import { devicePrices } from "./devicePrices";
...
export const allPlans: Plan[] = [
 ...
 /**
 * ===================================================
 * 📦 devicePrices.ts から自動で返却プログラム情報を紐付け（改良版）
 * ---------------------------------------------------
 * - planType に依存せず、devicePrices に登録されていれば紐づけ
 * - supportsReturnProgram は device が存在すれば true
 * ===================================================
 */
export const allPlansWithDevices: Plan[] = allPlans.map((plan) => {
  // 現時点では固定的に iPhone 17 Pro (512GB) を参照
  const device = devicePrices.find(
    (d) =>
  ...
  return {
    ...plan,
    deviceProgram: device
      ? {
          model: device.model,
          storage: device.storage,
          programName: device.programName,
          monthlyPayment: device.monthlyPayment,
          paymentMonths: device.paymentMonths,
          returnOption: device.returnOption,
          ownershipType: device.ownershipType,
        }
      : null,
    supportsReturnProgram: !!device,
