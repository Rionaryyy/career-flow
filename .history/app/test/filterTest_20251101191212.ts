...
mobileResult.forEach((plan, i) => {
  ...
  // 🆕 === 💰 還元額（円換算） ===
  console.log(`🪙 支払い還元額: ¥${(cost.paymentReward ?? 0).toLocaleString()}`);
  console.log(`🛒 ショッピング還元額: ¥${(cost.shoppingReward ?? 0).toLocaleString()}`);
  console.log(`🎁 ポイント還元額: ¥${(cost.pointReward ?? 0).toLocaleString()}`);
...
console.log("\n✅ 完了: Result.tsx + サブスク割 + 支払い割引 + 還元額表示 + 🌍海外通話 + 📞留守電オプション確認OK\n");
