...
const testAnswers: Partial<Phase2Answers> = {
  ...
  // 🎬 サブスク割確認用（追加）
  subscriptionList: ["Netflix", "YouTube Premium", "dTV"],
};
...
mobileResult.forEach((plan, i) => {
  ...
  if (plan.subscriptionDiscountRules && plan.subscriptionDiscountRules.length > 0) {
    const matchedSubs = plan.subscriptionDiscountRules.filter(rule =>
      rule.applicableSubscriptions.some(s =>
        (testAnswers.subscriptionList || []).includes(s)
      )
    );
    if (matchedSubs.length > 0) {
      console.log(`・サブスク割（${matchedSubs.map(r => r.applicableSubscriptions.join("・")).join(", ")}）:`);
      matchedSubs.forEach(sub => {
        console.log(`   → -¥${sub.discount}（${sub.id}）`);
      });
    } else {
      console.log("・サブスク割: 適用なし");
    }
  }
