...
// ✅ フェーズ①テスト回答（Phase1）
// ===================================================
const testDiagnosisAnswers: Partial<DiagnosisAnswers> = {
  includePoints: "はい",
...
// ✅ フェーズ②テスト回答（Phase2）
// ===================================================
const testAnswers: Partial<DiagnosisAnswers> = {
...
// 🟦 モバイルプラン抽出
// ===================================================
const mobileResult = filterPlansByPhase2(allPlans, testAnswers as DiagnosisAnswers);


// ===================================================
// 💡 セット割適用候補
// ===================================================
const fiberResult = filterByFiberSet(testAnswers as DiagnosisAnswers, fiberPlans);
const routerResult = filterByRouterSet(testAnswers as DiagnosisAnswers, routerPlans);
const pocketResult = filterByPocketWifiSet(testAnswers as DiagnosisAnswers, pocketPlans);
...
mobileResult.forEach((plan, i) => {
  const cost = calculatePlanCost(plan, { phase1: testDiagnosisAnswers, phase2: testAnswers } as any);
  ...
  if (plan.supportsGasSet && plan.energyDiscountRules)
    ...
    // === 🎬 サブスク割・還元（新DBから取得） ===
  if (cost.subscriptionDiscount || cost.subscriptionReward) {
    console.log("\n🎬 【サブスク特典】");

    if (cost.subscriptionDiscount && cost.subscriptionDiscount > 0)
      console.log(`・サブスクセット割合計: -¥${cost.subscriptionDiscount.toLocaleString()}`);

    if (cost.subscriptionReward && cost.subscriptionReward > 0)
      console.log(`・サブスク還元合計: -¥${cost.subscriptionReward.toLocaleString()}`);

    const totalSubBenefit = (cost.subscriptionDiscount ?? 0) + (cost.subscriptionReward ?? 0);
    console.log(`💡 合計サブスク特典（割引＋還元）: -¥${totalSubBenefit.toLocaleString()}`);
  }
...
if (
  cost.paymentReward ||
  cost.shoppingReward ||
  cost.carrierBarcodeReward ||
  cost.carrierShoppingReward
) {
  console.log("\n💴 【還元額詳細】");
...
const validation = validateDiagnosisResult(
  { phase1: testDiagnosisAnswers, phase2: testAnswers },
  mobileResult
);
