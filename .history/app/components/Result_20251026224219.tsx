          ...
          {rankedResults.map((plan, index) => (
            ...
            >
              ...
              <div className="mt-4 text-sm text-gray-700">
               ...
               {/* 💰 キャッシュバック詳細（compareAxisが"実際に支払う金額"の時のみ） */}
{compareAxis.includes("実際に支払う金額") && plan.breakdown.cashback !== 0 && (
  <div className="mt-2">
    <p>
      ・キャッシュバック(換算):{" "}
      {plan.breakdown.cashback >= 0 ? "-" : "+"}
      ¥{Math.abs(plan.breakdown.cashback)}
    </p>
    <p className="text-xs text-gray-500 ml-3">
      ↳ 総額: ¥{(plan.breakdown.cashbackTotal ?? 0).toLocaleString()} /{" "}
      月割: ¥{(plan.breakdown.cashback ?? 0).toLocaleString()}
    </p>
  </div>
)}

{/* 💸 初期費用詳細（compareAxisが"実際に支払う金額"の時のみ） */}
{compareAxis.includes("実際に支払う金額") && plan.breakdown.initialFeeMonthly !== 0 && (
  <div className="mt-1">
    <p>
      ・初期費用(月換算):{" "}
      {plan.breakdown.initialFeeMonthly >= 0 ? "+" : ""}
      ¥{plan.breakdown.initialFeeMonthly}
    </p>
    <p className="text-xs text-gray-500 ml-3">
      ↳ 総額: ¥{(plan.breakdown.initialCostTotal ?? 0).toLocaleString()}
    </p>
  </div>
)}
