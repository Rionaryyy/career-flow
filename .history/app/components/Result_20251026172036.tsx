    ...
    const withCosts: PlanWithCost[] = result.map(plan => {
      ...
      return {
        ...
        breakdown: {
          ...
          deviceBuyMonthly: (cost as any).deviceBuyMonthly ?? 0,
            ...
            >
              ...
              <div className="mt-4 text-sm text-gray-700">
                ...
                {plan.breakdown.deviceLeaseMonthly && plan.breakdown.deviceLeaseMonthly > 0 ? (
                  ...
                  <>
                    <p className="font-medium text-indigo-700">
                      ・端末代（一括購入想定・月換算）: +¥{plan.breakdown.deviceBuyMonthly}
                    </p>
                    <p className="text-xs text-gray-500 ml-2">
                      → 一括価格の目安: 約¥
                      {(plan.breakdown.deviceBuyMonthly * 24).toLocaleString()}（税込）
                    </p>
                  </>
                ) : null}
              ...
              {(answers.phase2?.deviceModel || answers.phase2?.deviceStorage) && (
                <div className="mt-2 text-xs text-gray-600 border-t border-dashed border-gray-300 pt-1">
                  ...
                  {plan.deviceProgram?.paymentMonths && `（${plan.deviceProgram.paymentMonths}ヶ月返却前提）`}
                </div>
              )}
            </div>
          ))}
