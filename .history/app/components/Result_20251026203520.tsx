...
import { allPlansWithDevices as allPlans } from "@/data/plans"; // ✅ 返却・購入対応版
...
interface PlanWithCost extends Plan {
  breakdown: {
    ...
    deviceBuyMonthly?: number; // 🆕 端末購入費用
    ...
    const withCosts: PlanWithCost[] = result.map(plan => {
      ...
      return {
        ...
        breakdown: {
          ...
          deviceLeaseMonthly: (cost as any).deviceLeaseMonthly ?? 0,
          deviceBuyMonthly: (cost as any).deviceBuyMonthly ?? 0, // 🆕 追加
          ...
          pointReward: (cost as any).pointReward ?? 0,
        },
          ...
          {rankedResults.map((plan, index) => (
            ...
            >
              ...
              <div className="mt-4 text-sm text-gray-700">
                ...
                {(plan.breakdown as any).deviceBuyMonthly &&
                  (plan.breakdown as any).deviceBuyMonthly > 0 && (
                    <p className="font-medium text-sky-700">
                      ・端末購入（月額端末費）: +¥{(plan.breakdown as any).deviceBuyMonthly}
                    </p>
                  )}
              ...
              {(answers.phase2?.deviceModel || answers.phase2?.deviceStorage) && (
                <div className="mt-2 text-xs text-gray-600 border-t border-dashed border-gray-300 pt-1">
                  ...
                  /{" "}
                  {plan.deviceProgram?.programName ??
                    (plan.breakdown.deviceBuyMonthly ? "キャリア端末購入（所有）" : "返却プログラム")}
                  {plan.deviceProgram?.paymentMonths &&
                    `（${plan.deviceProgram.paymentMonths}ヶ月${plan.breakdown.deviceBuyMonthly ? "分割払い" : "返却前提"}）`}
                </div>
              )}
            </div>
          ))}
