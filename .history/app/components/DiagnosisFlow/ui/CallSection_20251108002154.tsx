...
const PaymentSection = forwardRef(function PaymentSection(
  ...
  // 🔸 「同ページ内で表示する補足質問」
  const inlineFollowups = ["cardDetail"];
  ...
  useImperativeHandle(ref, () => ({
    goNext() {
      ...
      if (nextIndex < paymentQuestions.length) {
    ...
    isCompleted() {
      return (
        showExplanationOnly ||
        findNextIndex(currentIndex + 1) >= paymentQuestions.length
      );
    },
    getCurrentIndex() {
      return currentIndex;
  ...
  // 🔍 ページ送り時に補足質問（cardDetail）をスキップ
  function findNextIndex(startIndex: number) {
    let idx = startIndex;
    while (idx < paymentQuestions.length) {
      const q = paymentQuestions[idx];
      if (inlineFollowups.includes(q.id)) {
        idx++; // ページ送り対象外（同ページ内で出す）
      } else {
        break;
      }
    }
