...
export default forwardRef(function CallSection(
  ...
  return (
    <div className="w-full py-6 space-y-8">
      ...
      {/* === 🌍 海外通話：conditionがtrueの時だけ出す === */}
      {callQuestions
        .filter(
          (q) =>
            q.id === "needInternationalCallUnlimited" &&
            (!q.condition || q.condition(answers))
        )
        .map((q) => (
          <div key={q.id} className={outerCard}>
            ...
            {answers.needInternationalCallUnlimited === "はい" &&
              callQuestions
                .filter((s) => s.id === "internationalCallCarrier")
                .map((s) => (
