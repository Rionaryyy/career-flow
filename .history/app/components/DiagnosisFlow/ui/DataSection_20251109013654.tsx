...
const DataSection = forwardRef(function DataSection(
  ...
  const handleChange = (
    ...
    const updated: Partial<DiagnosisAnswers> = { [id]: value };

    // 🧩 テザリングを「いいえ」にしたら容量リセット
    if (id === "tetheringNeeded" && value === "いいえ") {
      updated.tetheringUsage = null;
    }

    // 🧩 データ使用量を「未選択」に戻した場合も速度重要度をリセット
    if (id === "dataUsage" && (value === "" || value === null)) {
      updated.speedLimitImportance = null;
    }
