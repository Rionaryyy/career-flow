...
const CallSection = forwardRef(function CallSection(
  ...
  const confirmValue = String(answers.needCallPlanConfirm || ""); // ← 追加

  const showUnknownFollowups =
    q.id === "needCallPlan" && needCallValue.includes("よくわからない");

  // ✅ 修正ポイント：「needCallPlanConfirm」もYES判定に含める
    ...
    <div className="w-full py-6 space-y-6">
      ...
      {showUnknownFollowups && (
        <div className={`${outerCard}`}>
          ...
          {callQuestions.filter((f) => isUnknownFollowupId(f.id)).map((sub) => (
            <div key={sub.id} className={innerCard}>
              <p className={`${bodyText} mb-2`}>{sub.question}</p>
              <OptionButtons id={sub.id as keyof DiagnosisAnswers} options={sub.options} />
            </div>
          ))}
        ...
        <div className={`${outerCard}`}>
          ...
          {callQuestions.filter((f) => isYesFollowupId(f.id)).map((sub) => (
            <div key={sub.id} className={innerCard}>
              ...
              {sub.id === "callPlanType" ? (
                <OptionButtons id={sub.id as keyof DiagnosisAnswers} options={sub.options} type="checkbox" />
              ) : (
                <OptionButtons id={sub.id as keyof DiagnosisAnswers} options={sub.options} type="radio" />
              )}
            </div>
          ))}
        </div>
