...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  // 🔹 BasicConditions と CallSection 用の ref（カード制御）
  ...
  const handleNext = () => {
    ...
    // 🐾 BasicConditions カード制御
    if (currentId === "basic" && basicRef.current) {
    ...
    // 🐾 CallSection カード制御
    if (currentId === "call" && callRef.current) {
      if (!callRef.current.isCompleted()) {
