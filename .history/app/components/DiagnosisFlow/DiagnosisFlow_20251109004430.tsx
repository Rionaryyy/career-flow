...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  useEffect(() => {
    if (!lastChangedKey) return; // 初回スキップ
    if (lastLogged.current === lastChangedKey) return;
    lastLogged.current = lastChangedKey;

    // 🧩 フィルター関数を自動判別
    const phase1Keys = ["carrierType", "networkQuality", "contractMethod", "supportPreference"];
    const isPhase1 = phase1Keys.includes(lastChangedKey);

    const filtered = isPhase1
      ? filterPlansByPhase1(allPlans, answers)
      : filterPlansByPhase2(allPlans, { phase2: answers } as any);

    const total = allPlans.length;

    console.log(`🧩 [${lastChangedKey}] 回答変更`);
    console.log(`📊 残り ${filtered.length} 件 / 全 ${total} 件`);
    console.log(`📉 ${total - filtered.length} 件減少\n`);
  }, [answers, lastChangedKey]);
