...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  const updateAnswers = (updated: Partial<DiagnosisAnswers>) => {
    ...
    const phase1Keys = ["carrierType", "networkQuality", "contractMethod", "supportPreference"];
    const isPhase1 = phase1Keys.includes(changedKey);

    // 🧹 リセットルール
    const tempAnswers = { ...answers, ...updated };
    if (changedKey === "tetheringNeeded" && changedValue === "いいえ") {
      tempAnswers.tetheringUsage = null;
      console.log("🧹 tetheringUsage をリセットしました");
    }

    // === 📊 フィルター適用 ===
    const filtered = isPhase1
      ? filterPlansByPhase1(allPlans, tempAnswers)
      : filterPlansByPhase2(allPlans, { phase2: tempAnswers } as any);
    ...
    const avg = costs.length
      ? costs.reduce((sum, c) => sum + (c.totalWithDevice ?? c.total ?? 0), 0) / costs.length
      : 0;
    ...
    // 件数変化
    if (countDiff < 0) console.log(`📉 ${Math.abs(countDiff)} 件減少 (${prevCount} → ${currentCount})`);
    else if (countDiff > 0) console.log(`📈 ${countDiff} 件増加 (${prevCount} → ${currentCount})`);
