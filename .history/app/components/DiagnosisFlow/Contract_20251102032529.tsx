...
export default function Phase2Contract({ answers, onChange }: Props) {
  // 質問定義（studentDiscount は特別枠でラップするため map 通常描画はスキップする）
  const questions = [
    {
      id: "familyLines",
    ...
    // studentDiscount は下で専用ボックスに入れて描画するため配列には残すが通常描画はしない
    {
      id: "studentDiscount",
      question: "学生ですか？",
      options: ["はい", "いいえ"],
  ...
  return (
    <div className="w-full py-6 space-y-6">
      {questions.map((q) => {
        // studentDiscount は通常描画しない（ageGroup の下で専用ボックスに入れて描画）
        if (q.id === "studentDiscount") return null;
            ...
            {q.id === "familyLines" && answers.familyLines === "4回線以上" && (
              <div className="mt-6">
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3 }}
                  className="relative w-full bg-white border border-pink-200 rounded-2xl p-5 pt-12 space-y-4"
                >
