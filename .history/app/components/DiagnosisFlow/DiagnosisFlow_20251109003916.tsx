...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  // === 🧩 回答更新処理（ここではstate更新のみ） ===
  const updateAnswers = (updated: Partial<DiagnosisAnswers>) => {
    setAnswers((prev) => ({ ...prev, ...updated }));
  };

  // === 🧠 変更検出＋件数ログ出力（useEffectでanswers更新後に実行） ===
  const lastLogged = useRef<string | null>(null);

  useEffect(() => {
    // 最後に変更されたキーを推測
    const changedKey = Object.keys(answers).at(-1);
    if (!changedKey) return;

    // StrictModeの2回実行を防止
    if (lastLogged.current === changedKey) return;
    lastLogged.current = changedKey;

    // 件数フィルター
    const filtered = filterPlansByPhase2(allPlans, { phase2: answers } as any);
    const total = allPlans.length;

    console.log(`🧩 [${changedKey}] 回答変更`);
