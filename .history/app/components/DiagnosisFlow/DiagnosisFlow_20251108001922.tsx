...
interface SectionRef {
  ...
  isAtLastQuestion?: () => boolean; // ✅ 各セクションが最後の質問かどうかを返す
}
...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  const getNextButtonLabel = () => {
    const currentRef = refs[currentId as keyof typeof refs];
    const isSectionLast = currentRef?.current?.isAtLastQuestion?.() ?? false;
    const isOverallLast = step === questions.length - 1 && isSectionLast;
    return isOverallLast ? "結果を見る" : "次へ";
  };
