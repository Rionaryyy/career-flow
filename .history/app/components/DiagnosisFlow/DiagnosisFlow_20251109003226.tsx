...
export default function DiagnosisFlow({ onSubmit, defaultValues, onBack }: Props) {
  ...
  // 🧩 回答更新時にリアルタイムデバッグログを出すよう修正
  const updateAnswers = (updated: Partial<DiagnosisAnswers>) => {
    setAnswers((prev) => {
      const newAnswers = { ...prev, ...updated };

      // === 変更箇所特定 ===
      const changedKey = Object.keys(updated)[0] as keyof DiagnosisAnswers;
      const changedValue = updated[changedKey];

      // === 件数フィルタリング ===
      const filtered = filterPlansByPhase2(allPlans, { phase2: newAnswers } as any);
      ...
      // ❗ 除外プランを見たい場合は下を有効化
      // const removed = allPlans.filter((p) => !filtered.includes(p));
      // console.table(removed.map((p) => ({ carrier: p.carrier, plan: p.planName })));
  ...
  const commonProps: FlowSectionProps = {
    answers,
    onChange: updateAnswers, // 🧩 ← ここで差し替え済み
    onNext: handleNext,
    onBack: handleBack,
  };
