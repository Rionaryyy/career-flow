 ...
 * - Phase1Answers → DiagnosisAnswers に統一
...
const basicQuestions = [
  ...
  },
  {
    ...
    question: "契約予定のサブスクリプション料金や割引も“実質料金”に含めて比較しますか？",
    type: "radio",
    options: [
...
export default function BasicConditions({ defaultValues, onSubmit }: Phase1Props) {
  const [answers, setAnswers] = React.useState<DiagnosisAnswers>(defaultValues);

  React.useEffect(() => {
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);

  const handleChange = (id: string | number | symbol, value: string | number | string[]) => {
  setAnswers((prev) => ({ ...prev, [id as string]: value as string }));
};

  // 🟦 フェーズ構成廃止後の単体版（にゃんこボタン・進捗バーは上位で制御）
  return (
