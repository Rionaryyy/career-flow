...
import { calculatePlanCost } from "@/utils/logic/calcEffectivePrice"; // 関数をインポート
...
import { allPlansWithCarrierData } from "@/data/allPlansWithCarrierData";
import { allCampaigns } from "@/data/allCampaigns";
...
export default function ResultContainer({ answers, onRestart }: Props) {
  const rankedResults = useMemo(() => {
    ...
    if (Object.values(answers).some((v) => v)) {
      result = filterPlans(result, answers);
      console.log("📍 統合フィルター後:", result.length);
    }
    ...
    const withCosts = result.map((plan: Plan) => {
      ...
      // デバッグ: 料金計算結果をログ出力
      console.log(`📊 Plan Cost for ${plan.planName}:`, cost);
