    ...
    <div className="ml-2 font-medium text-sky-700 mt-1">
      ...
      {(() => {
        ...
        return ` ${sign}¥${Math.abs(total).toLocaleString()}/月`;
               ...
               {/* 🎯 キャンペーン詳細表示ブロック */}
{plan.breakdown?.campaignMatched && plan.breakdown.campaignMatched.length > 0 && (
  <div className="mt-4 border-t border-dashed border-gray-300 pt-3">
    <p className="font-semibold text-gray-800 mb-1">🎯 適用キャンペーン</p>

    {plan.breakdown.campaignMatched.map((id: string) => {
      const matched = campaigns.find(c => c.campaignId === id);
      if (!matched) return null;
      return (
        <div key={matched.campaignId} className="ml-2 mb-2 text-sm text-gray-700">
          <p className="font-semibold text-sky-700">{matched.campaignName}</p>
          <p className="text-gray-600 text-xs">{matched.carrier}</p>
          <p className="text-gray-700 mt-1">
            💰 還元額:{" "}
            <span className="font-medium">
              {matched.cashbackAmount.toLocaleString()}円
            </span>{" "}
            （{matched.cashbackType}）
          </p>
          <p className="text-gray-600 text-xs mt-1">
            条件: {matched.conditions.join("・")}
          </p>
          <p className="text-gray-500 text-xs mt-1">{matched.description}</p>
        </div>
      );
    })}
  </div>
