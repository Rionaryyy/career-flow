...
import { allPlansWithDevices as allPlans } from "@/data/plans"; // ✅ 返却プログラム対応版を参照
...
interface PlanWithCost extends Plan {
  breakdown: {
    ...
    deviceBuyMonthly?: number; // ✅ 正規店購入用
    ...
    const withCosts: PlanWithCost[] = result.map(plan => {
      ...
      return {
        ...
        breakdown: {
          ...
          deviceBuyMonthly: (cost as any).deviceBuyMonthly ?? 0, // ✅ 正規店購入（月換算）
          ...
          {rankedResults.map((plan, index) => (
            ...
            >
              ...
              <div className="mt-4 text-sm text-gray-700">
                ...
                {/* ✅ 正規店購入（一括） */}
                {(plan as any).breakdown.deviceBuyMonthly &&
                  (plan as any).breakdown.deviceBuyMonthly > 0 && (
                    <>
                      <p className="font-medium text-indigo-700">
                        ・端末代（一括購入想定・月換算）: +¥
                        {(plan as any).breakdown.deviceBuyMonthly}
                      ...
                      <p className="text-xs text-gray-500 ml-2">
                        → 一括価格の目安: 約¥
                        {((plan as any).breakdown.deviceBuyMonthly * 24).toLocaleString()}（税込）
                      </p>
                    </>
                  )}
              ...
              {(answers.phase2?.deviceModel || answers.phase2?.deviceStorage) && (
                <div className="mt-2 text-xs text-gray-600 border-t border-dashed border-gray-300 pt-1">
                  ...
                  /{" "}
                  {plan.deviceProgram?.programName ?? "返却プログラム"}
                  {plan.deviceProgram?.paymentMonths &&
                    `（${plan.deviceProgram.paymentMonths}ヶ月返却前提）`}
                </div>
              )}
            </div>
          ))}
