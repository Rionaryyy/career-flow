...
{(plan.breakdown.subscriptionDetails?.length ?? 0) > 0 && (
  <div className="mt-2">
    ...
    {/* 🧩 重複サブスク統合（セット割＋還元を1行にまとめる） */}
    {(() => {
      const mergedSubs = Object.values(
        (plan.breakdown.subscriptionDetails ?? []).reduce((acc, s) => {
          const key = s.name.replace(/（.*?）/g, "").trim(); // 「（セット割）」等を除去して統合キー化
          if (!acc[key]) acc[key] = { ...s };
          else {
            acc[key].discount = (acc[key].discount ?? 0) + (s.discount ?? 0);
            acc[key].reward = (acc[key].reward ?? 0) + (s.reward ?? 0);
          }
          return acc;
