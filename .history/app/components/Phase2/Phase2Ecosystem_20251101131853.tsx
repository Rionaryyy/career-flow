...
export default function Phase2Ecosystem({ answers, onChange, phase1Answers, onNext }: Props) {
  ...
  const questions = [
    ...
    {
      id: "shoppingMonthly",
      question: "日々のショッピング（月あたり）の想定利用額をスライダーで選んでください",
      type: "slider" as const,
      min: 1000,
      max: 200000,
      step: 1000,
      unit: "円",
      options: [], // ← 明示的に空配列にして型一致させる
    ...
    },
{
  ...
  options: [
    "楽天Pay / 楽天カード（楽天経済圏）",
    "楽天ゴールドカード / 楽天Pay（楽天経済圏・上位カード）",
    "d払い / dカード（dポイント経済圏）",
    "dカード GOLD / d払い（dポイント経済圏・上位カード）",
    "PayPay / ペイペイカード（PayPay経済圏）",
    "PayPayゴールドカード（PayPay経済圏・上位カード）",
    "au PAY / au PAYカード（Ponta経済圏）",
    "au PAY ゴールドカード（Ponta経済圏・上位カード）",
    "どれが一番お得か分からないので、すべてのパターンを比較したい",
    "特になし",
  ],
  type: "checkbox" as const,
    ...
    {
      id: "paymentMonthly",
      question: "日常の支払い（月あたり）の想定利用額をスライダーで選んでください",
      type: "slider" as const,
      min: 1000,
      max: 200000,
      step: 1000,
