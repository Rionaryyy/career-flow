...
interface PlanWithCost extends Plan {
  breakdown: {
    ...
    electricDiscount?: number; // ⚡ 電気
    gasDiscount?: number; // 🔥 ガス
    subscriptionDiscount?: number; // 🎬 サブスク割引追加
    paymentDiscount?: number; // 💳 支払い方法割引
    paymentReward?: number; // 💳 支払い還元
  };
  totalMonthly: number;
}
    ...
    const withCosts: PlanWithCost[] = result.map(plan => {
      ...
      return {
        ...
        breakdown: {
          ...
          electricDiscount: cost.electricDiscount ?? 0, // ⚡ 電気
          gasDiscount: cost.gasDiscount ?? 0, // 🔥 ガス
          subscriptionDiscount: cost.subscriptionDiscount ?? 0, // 🎬 サブスク割引追加
          paymentDiscount: cost.paymentDiscount ?? 0, // 💳 支払い方法割引
          paymentReward: cost.paymentReward ?? 0, // 💳 支払い還元
        },
            ...
            >
              ...
              <div className="mt-4 text-sm text-gray-700">
                ...
                {(plan.breakdown as any)["fiberDiscount"] !== 0 && (
                  <p>
                    ・光回線セット割: -¥
                    {(plan.breakdown as any)["fiberDiscount"]}
                  </p>
                )}
                {(plan.breakdown as any)["routerDiscount"] !== 0 && (
                  <p>
                    ・ルーター割引: -¥
                    {(plan.breakdown as any)["routerDiscount"]}
                  </p>
                )}
                {(plan.breakdown as any)["pocketWifiDiscount"] !== 0 && (
                  <p>
                    ・ポケットWi-Fi割: -¥
                    {(plan.breakdown as any)["pocketWifiDiscount"]}
                  </p>
                )}
                {/* ⚡ 電気・ガスセット割 */}
                {(plan.breakdown as any)["electricDiscount"] !== 0 && (
                  <p>
                    ・電気セット割: -¥
                    {(plan.breakdown as any)["electricDiscount"]}
                  </p>
                )}
                {(plan.breakdown as any)["gasDiscount"] !== 0 && (
                  <p>
                    ・ガスセット割: -¥
                    {(plan.breakdown as any)["gasDiscount"]}
                  </p>
                )}
                {/* 🎬 サブスク割 */}
                {(plan.breakdown as any)["subscriptionDiscount"] !== 0 && (
                  <p>
                    ・サブスク割: -¥
                    {(plan.breakdown as any)["subscriptionDiscount"]}
                  </p>
                )}
                {/* 💳 支払い系追加 */}
                {(plan.breakdown as any)["paymentDiscount"] !== 0 && (
                  <p>
                    ・支払い方法割引: -¥
                    {(plan.breakdown as any)["paymentDiscount"]}
                  </p>
                )}
                {(plan.breakdown as any)["paymentReward"] !== 0 && (
                  <p>
                    ・支払い還元（実質）: -¥
                    {(plan.breakdown as any)["paymentReward"]}
                  </p>
                )}
              </div>
