...
export interface DeviceProgramInfo {
  ...
  ownershipType: "lease" | "buy"; // ✅ 修正: 正規店購入対応
  totalPayment?: number;
}
...
export interface Plan {
  ...
  shoppingRewardRate?: number; // ショッピング利用に対する還元率（例：0.01 = 1%）
  paymentRewardRate?: number;  // 支払い利用に対する還元率（例：0.015 = 1.5%）
  carrierPaymentRewardRate?: number; // 例: 0.05 → 5%
  carrierPaymentRewardLimit?: number; // 月上限（円）
  carrierShoppingRewardRate_Yahoo?: number; // Yahoo!ショッピング等
  ...
  includedSubscriptions?: string; // 🎬 プランに付帯しているサブスク（カンマ区切り）
...
export interface PlanCostBreakdown {
  ...
  deviceBuyMonthly?: number; // ✅ 追加: 正規店購入分割用
}
