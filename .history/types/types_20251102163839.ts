export interface Phase1Answers {
  ...
  appCallUnlimited?: string | null; // ← optionalにして安全化
   includeSubscription?: string; // ← これを追加！
...
export interface Phase2Answers {
  ...
  // 🟦 新規追加（経済圏詳細構造）
  paymentEcosystem?: string[]; // 経済圏選択（楽天 / d / PayPay / au）
  ...
  subscriptionMode?: string; // ← 新しい質問IDに対応
  ...
  cardDetail?: string[];    // 💳 支払いカード・銀行の詳細を追加 ←これ！
