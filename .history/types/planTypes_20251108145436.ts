...
export interface Plan {
  ...
  carrierShoppingRewardRate_PayPayMall?: number; // 🆕 追加: PayPayモール／PayPay経済圏還元率
...
export interface Campaign {
  campaignName: string; // 表示用タイトル
  campaignId: string; // 一意のID
  carrier: string; // キャリア名
  targetPlanIds: string[]; // 対象プランID群
  cashbackAmount: number; // 還元額
  cashbackType: string; // ポイント種別など
  conditions: string[]; // 適用条件群
